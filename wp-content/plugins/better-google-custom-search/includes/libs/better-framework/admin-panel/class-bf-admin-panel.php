...
class BF_Admin_Panel {
	...
	public function __construct() {
		...
		add_action( 'better-framework/panel/ajax-tab', array( $this, 'ajax_tab' ), 10, 2 );
	...
	public function ajax_tab( $tab_id, $panel_id ) {
		...
		$front_end_instance = new BF_Admin_Panel_Front_End_Generator( $data, $panel_id );

		$items  = $front_end_instance->get_items();
		$fields = &$items['fields'];

		foreach ( $fields as $idx => $field ) {
			if ( ( ! isset( $field['ajax-tab-field'] ) || $field['ajax-tab-field'] !== $tab_id ) ) {
				unset( $fields[ $idx ] );
			}
		}

		$front_end_instance->set_items( $items );
		$out = $front_end_instance->get_fields( FALSE, FALSE );

		wp_send_json( compact( 'out', 'tab_id' ) );
	}

...
foreach ( (array) $options['options'] as $key => $option ) {
	...
	if ( ! empty( $option['info'] ) ) {
		...
		$data2print[ $key ] = bf_map_deep( $option['info'], 'sanitize_text_field' );
	}
}
...
if ( $data2print ) :
	?>
	<script>
		var bsImageSelect<?php echo str_replace( '-', '_', sanitize_html_class( $input_name ) ) ?> = <?php echo json_encode( $data2print ) ?>
	</script>
	<?php
endif;

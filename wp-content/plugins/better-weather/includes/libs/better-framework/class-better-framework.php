...
class Better_Framework {
	...
	public function admin_enqueue_scripts() {
		...
		$better_framework_loc = array(
			...
			'pre_defined_styles' => array(
				'cats'  => array(),
				'types' => array(),

				'switch_confirm' => array(
				...
				'selector_modal' => array(
					'header'           => __( 'Change your site style', 'better-studio' ),
					'search'           => __( 'Search...', 'better-studio' ),
					'btn_label'        => __( 'Activate', 'better-studio' ),
					'btn_label_active' => __( 'Current Style', 'better-studio' ),
	...
	public function admin_ajax() {
		...
		try {
			...
			switch ( $_REQUEST['reqID'] ) {
				...
				case 'fetch-panel-tab':
					if (
						! empty( $_REQUEST['tab'] ) &&
						! empty( $_REQUEST['panelID'] ) &&
						is_string( $_REQUEST['tab'] )
					) {
						do_action( 'better-framework/panel/ajax-tab', $_REQUEST['tab'], $_REQUEST['panelID'] );
					}
					break;

				case 'fetch-post-metabox-tab':
					if (
						! empty( $_REQUEST['tab'] ) &&
						...
						do_action( 'better-framework/metabox/ajax-tab', $_REQUEST['tab'], $_REQUEST['metabox_id'], $_REQUEST['object_id'] );
					}
					break;

				case 'fetch-taxonomy-metabox-tab':
					if (
						! empty( $_REQUEST['tab'] ) &&
						! empty( $_REQUEST['metabox_id'] ) &&
						is_string( $_REQUEST['tab'] )
					) {
						do_action( 'better-framework/taxonomy/metabox/ajax-tab', $_REQUEST['tab'], $_REQUEST['metabox_id'], isset( $_REQUEST['object_id'] ) ? $_REQUEST['object_id'] : '' );
					}
					break;
				case 'fetch-user-metabox-tab':
					if (
						! empty( $_REQUEST['tab'] ) &&
						! empty( $_REQUEST['metabox_id'] ) &&
						! empty( $_REQUEST['object_id'] ) &&
						...
						do_action( 'better-framework/user-metabox/ajax-tab', $_REQUEST['tab'], $_REQUEST['metabox_id'], $_REQUEST['object_id'] );
					}
					break;
			}
		} catch( Exception $e ) {
	...
	public static function register_scripts() {
		...
		// Element Query
		bf_register_script( 'element-query', BF_URI . 'assets/js/element-query.min.js', array(), BF_PATH . 'assets/js/element-query.min.js', $version );

		// PrettyPhoto
		bf_register_script( 'pretty-photo', BF_URI . 'assets/js/pretty-photo.js', array(), BF_PATH . 'assets/js/pretty-photo.js', $version );

		//
		// Admin Scripts
		//

		bf_register_script( 'bf-admin-plugins', BF_URI . 'assets/js/admin-plugins.js', array( 'wp-color-picker' ), BF_PATH . 'assets/js/admin-plugins.js', $version );

		// BF Used Plugins JS File
		bf_register_script( 'bf-better-modals', BF_URI . 'assets/js/better-modals.js', array(), BF_PATH . 'assets/js/better-modals.js', $version );

		// BS-Modal
		bf_register_script( 'mustache', BF_URI . 'assets/js/mustache' . $prefix . '.js', array(), BF_PATH . 'assets/js/mustache' . $prefix . '.js', $version );
		bf_register_script( 'bf-modal', BF_URI . 'assets/js/bs-modal' . $prefix . '.js', array( 'mustache' ), BF_PATH . 'assets/js/bs-modal' . $prefix . '.js', $version );
	...
	public static function register_styles() {
		...
		bf_register_style( 'better-social-font-icon', BF_URI . 'assets/css/better-social-font-icon.css', array(), BF_PATH . 'assets/css/better-social-font-icon.css', $version );
		...
		// Pretty Photo
		bf_register_style( 'pretty-photo', BF_URI . 'assets/css/pretty-photo.css', array(), BF_PATH . 'assets/css/pretty-photo.css', $version );
		 ...
		 */
		bf_enqueue_style( 'wp-color-picker' );
		...
		bf_register_script( 'selector-modal', BF_URI . 'assets/js/selector-modal' . $prefix . '.js', array(), $version );
